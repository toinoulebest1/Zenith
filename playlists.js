...
function getPlaylistCoverHTML(playlist) {
    ...
    if(items.length < 4) {
        ...
        let url = t.source === 'subsonic' ? `${API_BASE}/get_subsonic_cover/${t.album.image.large}` : (t.album.image.large || '').replace('_300', '_600');
...
function renderPlaylistGrid(items, isOwner, playlistId) {
    ...
    g.innerHTML = '';
    items.forEach(it => {
        let u = it.source === 'subsonic' ? `${API_BASE}/get_subsonic_cover/${it.album.image.large}` : (it.album.image.large || '').replace('_300', '_600');
